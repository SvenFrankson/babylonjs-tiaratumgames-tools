...
interface ISceneObject {
    ...
    export class AnimationFactory {
        ...
        public static EmptyVoidCallback: (duration: number) => Promise<void> = async () => {};
        public static EmptyNumberCallback: (target: number, duration: number, overrideEasing?: (v: number) => number) => Promise<void> = async () => {};
        public static EmptyNumbersCallback: (targets: number[], duration: number) => Promise<void> = async () => {};
        public static EmptyVector3Callback: (target: Vector3, duration: number, overrideEasing?: (v: number) => number) => Promise<void> = async () => {};
        public static EmptyColor3Callback: (target: Color3, duration: number, overrideEasing?: (v: number) => number) => Promise<void> = async () => {};
        public static EmptyQuaternionCallback: (target: Quaternion, duration: number, overrideEasing?: (v: number) => number) => Promise<void> = async () => {};
            ...
            return (target: number, duration: number, overrideEasing?: (v: number) => number) => {
                return new Promise<void>(resolve => {
                    ...
                    let animationCB = () => {
                        ...
                        if (f < 1) {
                            if (isAngle) {
                                obj[property] = LerpAngle(origin, target, f);
                            }
                            else {
                                if (overrideEasing) {
                                    f = overrideEasing(f);
                        ...
                        if (f < 1) {
                            ...
                            for (let i = 0; i < n; i++) {
                                if (isAngle && isAngle[i]) {
                                    obj[properties[i]] = LerpAngle(origins[i], targets[i], f);
                                }
                                else {
